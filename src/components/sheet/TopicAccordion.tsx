...
export function TopicAccordion({
  ...
  return (
    ...
    >
      ...
      >
        <AccordionTrigger className="px-4 sm:px-5 py-3 sm:py-4 hover:no-underline group flex items-center justify-between gap-2 sm:gap-4 bg-card hover:bg-muted/50 transition-colors [&[data-state=open]]:border-b [&[data-state=open]]:border-border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/10 focus-visible:ring-inset">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            <span className="hidden sm:block w-px self-stretch min-h-[20px] rounded-full bg-border group-data-[state=open]:bg-primary transition-colors" aria-hidden="true" />
            <h2 className="font-[var(--font-newsreader)] text-[15px] sm:text-[17px] md:text-[18px] tracking-[-0.02em] leading-none text-foreground text-left truncate" title={topic.name}>
              {topic.name}
            </h2>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="w-[72px] sm:w-[88px] h-2 rounded-full bg-secondary border border-border overflow-hidden hidden sm:flex" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label={`${pct}% of topic complete`}>
              <div className="h-full rounded-full bg-primary transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)]" style={{ width: `${pct}%`, minWidth: pct > 0 ? '2px' : '0' }} />
            </div>
        ...
        <AccordionContent className="px-2 pb-2 sm:px-2 sm:pb-2 md:px-3 md:pb-3 bg-muted/30">
